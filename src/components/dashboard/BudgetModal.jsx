...
import { prevYearMonth, toYearMonth } from "../../utils/formatters";

export function BudgetModal({ isOpen, onClose, budgets, transactions, saveBudget, currentYearMonth, allBudgets, allTransactions }) {
  ...
  const prevYM = prevYearMonth(currentYearMonth);
  const prevBudgets = (allBudgets || []).filter(b => b.yearMonth === prevYM);
  const prevMonthExpenses = (allTransactions || []).filter(t => {
    if (!t.date || t.type !== 'expense' || (t.isDebt && t.debtPaid)) return false;
    const tDate = new Date(t.date.seconds * 1000);
    return toYearMonth(tDate) === prevYM;
  });
              ...
              activeBudgets.map(macro => {
                ...
                return (
                  <BudgetRow
                    ...
                    prevLimit={prevBudget ? prevBudget.limit : null}
                    prevSpent={prevBudget ? prevSpent : null}
                  />
                );
              })
          )}
...
function BudgetRow({ macro, limit, spent, remaining, percentage, isOver, saveBudget, prevLimit, prevSpent }) {
    ...
    <div className={`p-3 rounded-xl border transition-colors ${isOver ? 'bg-red-500/5 border-red-500/30' : 'bg-gray-700/30 border-gray-700'}`}>
        ...
        {prevLimit !== null && (
            <div className="text-[10px] text-gray-500 mt-1.5 pt-1.5 border-t border-gray-700/50">
                Mês anterior: <span className="text-gray-400">R$ {prevSpent.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
                <span className="text-gray-600"> / </span>
                <span className="text-gray-400">R$ {prevLimit.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
            </div>
        )}
    </div>
  );
