...
export function PaymentOptions({
  ...
  return (
    <div className="space-y-2">
        ...
        {(paymentMethod === 'credit' || isDebt) && !isSubscription && (
            <div className={`p-2 rounded-lg border transition-all ${isInstallment ? 'bg-purple-500/10 border-purple-500' : 'bg-gray-700/30 border-gray-600'}`}>
                <div className={`flex items-center gap-2 ${isInstallment ? 'mb-2' : ''}`}>
                    ...
                    <label htmlFor="installCheck" className="text-sm text-gray-300 cursor-pointer select-none flex items-center gap-2 w-full"><Layers size={14} /> {isDebt && paymentMethod !== 'credit' ? 'Parcelar Dívida' : 'Parcelar Compra'}</label>
                </div>
                {isInstallment && (
